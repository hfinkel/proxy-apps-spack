...
class Aspa(MakefilePackage):
    ...
    demands upon the exascale ecosystem that need to be identified and quantified.
    tags = proxy-app
    """
    tags = ['proxy-app']
    ...
    version('master', git='https://github.com/exmatex/ASPA.git',description='master')
    ...
    def build_targets(self):
        targets = [
            '--directory=exec',
            '--file=Makefile',
            'LIBS={0} {1} {2}'.format(self.spec['lapack'].libs.ld_flags,self.spec['blas'].libs.ld_flags,self.spec['hdf5'].libs.ld_flags),
